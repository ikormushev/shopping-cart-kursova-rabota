...
public class KauflandService {
  ...
  private BigDecimal extractPrice(String line) {
    ...
    if (matcher.find()) {
      try {
        ...
        System.err.println("Failed to parse price: " + matcher.group(1));
      }
    }
  ...
  private void saveProductAndPrice(String name, BigDecimal price) {
    String cleanName = name.trim();
    if (cleanName.isBlank()) return;

    try {
      // 1. Проверяваме дали продуктът вече съществува
      ProductEntity product = productMapper.findByRawNameAndStore(cleanName, "KAUFLAND");

      if (product == null) {
        product = new ProductEntity();
        product.setId(UUID.randomUUID().toString());
        product.setRawName(cleanName);
        product.setSku(cleanName);
        product.setPrice(price);
        product.setCreatedAt(OffsetDateTime.now());
        product.setStoreId(storeMapper.findByName("KAUFLAND"));
        product.setStoreName("KAUFLAND");
        try {
          productMapper.insert(product);
        } catch (Exception e) {
          // Ако гръмне тук, значи друга нишка го е създала току-що -> взимаме го
          product = productMapper.findByRawNameAndStore(cleanName, "KAUFLAND");
          if (product == null) return;
        }
      } else {
        productMapper.updatePrice(product.getId(), price);
      }
