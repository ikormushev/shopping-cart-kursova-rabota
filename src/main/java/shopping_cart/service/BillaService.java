...
//  private static final UUID BILLA_STORE_ID =
//      UUID.fromString("00000000-0000-0000-0000-000000000003");
//
//  @Value("${billa.url}")
//  private String brochurePageUrl;
//
//  public BillaService(ChromeDriver driver, ProductMapper productMapper, PriceMapper priceMapper) {
//    this.driver = driver;
...
//    System.out.println(
//        ">>> BILLA SERVICE: Започва сваляне на брошура от homepage (с навигация до слайдера)");
//    try {
//      priceMapper.deletePricesByStoreId(BILLA_STORE_ID.toString());
...
//
//      WebElement firstTeaser =
//          slider.findElement(
//              By.cssSelector(
//                  "div.ws-teaser__content.pa-4:first-of-type, .ws-teaser__content:first-child"));
...
//      }
//
//      WebElement brochureLink =
//          firstTeaser.findElement(By.tagName("a")); // Или By.cssSelector("a[href*='promocii']")
...
//          .executeScript("arguments[0].scrollIntoView(true);", brochureLink);
//      Thread.sleep(1000);
...
//    } catch (Exception e1) {
//      System.out.println(
//          "Навигация през слайдера не сработи: "
//              + e1.getMessage()
//              + " – fallback към директна навигация.");
//
//      try {
...
//                      By.xpath(
//                          "//a[contains(text(), 'Промоции') or contains(@href, 'promocii')]")));
//          menuLink.click();
//          Thread.sleep(2000);
//          // После клик на брошура
//          WebElement brochureMenu =
//              wait.until(
//                  ExpectedConditions.elementToBeClickable(
...
//              "Не може да се навигира до брошурата! Провери homepage структурата.", e3);
//        }
//      }
//    }
//
...
//                ExpectedConditions.presenceOfElementLocated(
//                    By.cssSelector(
//                        "iframe[src*='publitas.com'], iframe[src*='viewer'], iframe[class*='publication']")));
...
//                By.xpath(
//                    "//a[contains(@href, '.pdf') or contains(@href, 'publitas') and (contains(., 'PDF') or contains(., 'Изтегли'))]"));
//
...
//        String apiUrl =
//            "https://www.billa.bg/api/brochure/pdf/current"; // Или конкретния от твоя snippet
//        driver.get(apiUrl);
//      }
//
//      if (inIframe) {
...
//      System.err.println(
//          "!!! ГРЕШКА: PDFTextStripper върна празен текст! Този PDF е само картинки (Scanned). Трябва OCR.");
//      return;
//    }
//
...
//        Pattern.compile("(\\d+[.,]\\d{2})\\s*(?:лв|BGN|€)?", Pattern.CASE_INSENSITIVE);
//
...
//
//            // Стоп думи
//            if (prev.matches("(?i).*(лв|bgn|цена|отстъпка|супер|промо|без кост|за 1 кг).*"))
//              continue;
...
//    name =
//        name.replaceAll("(?i)(Весела Коледа|25 години|ВИНАГИ ДО ВАС|СЕГА В BILLA|ПРАЗНУВАЙ С)", "");
...
//  }
//
//  private Rectangle floodFillColorBlock(
//      BufferedImage img, int sx, int sy, boolean[][] visited, Color target) {
...
//  }
//
//  private File waitForPdfDownload(File downloadDir, int timeoutSeconds)
//      throws InterruptedException {
//    long deadline = System.currentTimeMillis() + timeoutSeconds * 1000L;
//    while (System.currentTimeMillis() < deadline) {
//      File[] files =
//          downloadDir.listFiles(
//              (dir, name) -> name.toLowerCase().endsWith(".pdf") && !name.endsWith(".crdownload"));
//
