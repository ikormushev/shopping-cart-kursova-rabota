...
public class UnifiedBasketFacade {
                        ...
                        .map(
                                entity ->
                                        BasketItem.builder()
                                                ...
                                                .checked(Boolean.TRUE.equals(entity.getChecked()))
                                                .build())
                        .toList();
    ...
    @Transactional
    public void toggleItemChecked(String sessionId, String itemId, boolean checked) {
        getSession(sessionId);
        basketService.toggleItemChecked(itemId, checked);
    }

    @Transactional
    public void checkAllItems(String sessionId, String basketId, boolean checked) {
        Session session = getSession(sessionId);
        validateMembership(basketId, session.getUserId());
        basketService.checkAllItems(basketId, checked);
    }

    public List<BasketItemEntity> getCheckedItems(String sessionId) {
        Session session = getSession(sessionId);
        return basketService.findCheckedItemsByBasketId(session.getCartId().toString());
    }
}
